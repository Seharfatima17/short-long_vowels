...
export default function LetterActivityScreen() {
  ...
  useEffect(() => {
    const completed = correctWords.every(word => selectedWords.includes(word));
    if (completed && !showSuccess) {
      Speech.speak(`🎉 Excellent! You found all words with ${letterString.toUpperCase()}`);
      setShowSuccess(true);
    }
  }, [selectedWords]);
      ...
      <View style={styles.wordGrid}>
        {wordData.map((item, index) => (
          <TouchableOpacity
            ...
            activeOpacity={1} // Prevent resizing on press
          >
