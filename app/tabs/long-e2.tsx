...
const LongEActivity = () => {
  ...
  const totalQuestions = longEWords.length;
  ...
  const handleGoNext = async () => {
    try {
      if (Object.keys(selected).length > 0) {
        await saveScore("long", "e", score, totalQuestions, selected);
      }
      navigation.navigate('homescreen');
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity style={styles.backArrow} onPress={() => navigation.navigate('long-e1')}>
        <Icon name="arrow-back" size={28} color="#333" />
      </TouchableOpacity>

      <View style={styles.header}>
        <TouchableOpacity style={styles.backButton} onPress={() => navigation.navigate('long-e1')}>
